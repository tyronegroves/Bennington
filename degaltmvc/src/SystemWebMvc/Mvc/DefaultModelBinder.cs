namespace System.Web.Mvc {
    ...
    public class DefaultModelBinder : IModelBinder {
        ...
        private static bool AreAllSubPropertiesValid(ModelStateDictionary dictionary, string prefix) {
            return !DictionaryHelpers.FindKeysWithPrefix(dictionary, prefix)
                                     .Any(kvp => kvp.Key.Length != prefix.Length && kvp.Value.Errors.Count > 0);
        }
        ...
        protected virtual void OnModelUpdated(ControllerContext controllerContext, ModelBindingContext bindingContext) {
            foreach (ModelValidationResult validationResult in ModelValidator.GetModelValidator(bindingContext.ModelMetadata, controllerContext).Validate(null)) {
                ...
                if (AreAllSubPropertiesValid(bindingContext.ModelState, subPropertyName)) {
        ...
        protected virtual bool OnPropertyValidating(ControllerContext controllerContext, ModelBindingContext bindingContext, PropertyDescriptor propertyDescriptor, object value) {
            // all of the validation logic has been moved to the OnPropertyValidated() method
            return true;
        }
