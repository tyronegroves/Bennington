...
namespace Paragon.ContentTree.SectionNodeProvider.Routing
{
    public class ContentTreeSectionRouteConstraint : IRouteConstraint
    {
    	...
    	public ContentTreeSectionRouteConstraint(ITreeNodeSummaryContext treeNodeSummaryContext)
    	{
		...
		{
			var children = treeNodeSummaryContext.GetChildren(parentTreeNodeId).Where(a => a.Type == typeof(SectionNodeProvider).AssemblyQualifiedName);
